...
from odoo.addons.or_base.helper.multi_threading import Threading
...
class AttendanceDevice(models.Model):
    ...
    protocol = fields.Selection([('udp', 'UDP'), ('tcp', 'TCP'), ('icloud', 'Icloud - ADMS')], string='Protocol',
                                required=True, default='tcp',
                                tracking=True,
    ...
    def _compute_device_users_count(self):
        total_att_data = self.env['attendance.device.user']._read_group([('device_id', 'in', self.ids)], ['device_id'],
                                                                        ['__count'])
    ...
    def _compute_total_finger_template_records(self):
        total_att_data = self.env['finger.template']._read_group([('device_id', 'in', self.ids)], ['device_id'],
                                                                 ['__count'])
        mapped_data = {device.id: count for device, count in total_att_data}
        for r in self:
    ...
    def _compute_total_attendance_records(self):
        total_att_data = self.env['user.attendance']._read_group([('device_id', 'in', self.ids)], ['device_id'],
                                                                 ['__count'])
        mapped_data = {device.id: count for device, count in total_att_data}
        for r in self:
    ...
    def _compute_employees(self):
        ...
        for r in self:
            r.update({
                'unmapped_employee_ids': [(6, 0, HrEmployee.search(
                    [('company_id', '=', r.company_id.id), ('id', 'not in', r.device_user_ids.employee_id.ids)]).ids)],
                'mapped_employee_ids': [
                    (6, 0, r.device_user_ids.employee_id.filtered(lambda employee: employee.active is True).ids)],
            })
    ...
    def write(self, vals):
        ...
        if any(field in vals for field in self._get_updated_fields_for_icloud_setting()) or (
                'protocol' in vals and vals['protocol'] == 'icloud'):
            for r in self:
    ...
    def connect(self):
        ...
        if error_msg:
            email_template = self.env.ref('pr_attendance_device.email_template_attendance_device')
    ...
    def getAttendance(self):
        ...
        try:
            ...
            if post_err_msg and self.download_error_notification:
                email_template_id = self.env.ref('pr_attendance_device.email_template_error_get_attendance')
    ...
    def _fetch_attendance_data(self, attendance_data_icloud=None):
        ...
        email_template = self.env.ref('pr_attendance_device.email_template_unknown_attendance_status_code')
        for r in self:
            error_msg = ""
            attendance_states = {}
            if r.map_before_dl:
            ...
            if r.last_attendance_download:
                ...
                attendance_data = [attendance for attendance in attendance_data if
                                   attendance.timestamp >= last_attendance_download]
            ...
            attendance_users_dict = {att['user_id']: att['id'] for att in
                                     AttendanceUser.with_context(active_test=False).search_read(
                                         [('user_id', 'in', attendance_user_ids), ('device_id', '=', r.id)],
                                         ['user_id'])}

            all_attendance_utc_timestamp = [r.convert_local_to_utc(attendance.timestamp, r.tz, naive=True) for
                                            attendance in attendance_data]
            exists_attend_data = {
                frozendict(
                    {'device_id': att['device_id'][0], 'user_id': att['user_id'][0], 'timestamp': att['timestamp']}):
                    att['id']
            ...
            for attendance in attendance_data:
                ...
                if attendance_user_id:
                    ...
                    if exists_attend_data.pop(
                            frozendict({'device_id': r.id, 'user_id': attendance_user_id, 'timestamp': utc_timestamp}),
                            False):
                        continue
    ...
    def _finger_template_download(self, fingers=None):
        ...
        for r in self:
            ...
            for template in template_data:
                ...
                size = template.size
                device_user_id = device_users.filtered(
                    lambda u: u.user_id == user_id) if r.protocol == 'icloud' else device_users.filtered(
                    lambda u: u.uid == uid)
                ...
                if r.protocol == 'icloud' and (
                f"{template.user_id}_{template.fid}") not in existing_finger_template_ids:
                    FingerTemplate.create(vals)
                else:
    ...
    def _attendance_clear(self):
        ...
        """
        email_template = self.env.ref('pr_attendance_device.email_template_not_safe_to_clear_attendance')
        for r in self:
            if r.protocol == 'icloud':
    ...
    def action_view_users(self):
        result = self.env['ir.actions.act_window']._for_xml_id('pr_attendance_device.device_user_list_action')
        ...
        if self.device_users_count != 1:
            ...
            res = self.env.ref('pr_attendance_device.attendance_device_user_form_view', False)
    ...
    def action_view_attendance_data(self):
        self.ensure_one()
        result = self.env['ir.actions.act_window']._for_xml_id('pr_attendance_device.action_user_attendance_data')
        ...
        if total_att_records != 1:
            ...
            res = self.env.ref('pr_attendance_device.view_attendance_data_form', False)
            result['views'] = [(res and res.id or False, 'form')]
    ...
    def action_view_mapped_employees(self):
        ...
        if self.mapped_employees_count != 1:
            result['domain'] = "[('id', 'in', " + str(
                self.with_context(active_test=False).mapped_employee_ids.ids) + ")]"
        elif self.mapped_employees_count == 1:
            res = self.env.ref('pr_attendance_device.view_employee_form', False)
            result['views'] = [(res and res.id or False, 'form')]
    ...
    def action_view_finger_template(self):
        self.ensure_one()
        result = self.env['ir.actions.act_window']._for_xml_id('pr_attendance_device.action_finger_template')
        ...
        if total_finger_template_records != 1:
            ...
            res = self.env.ref('pr_attendance_device.view_finger_template_form', False)
            result['views'] = [(res and res.id or False, 'form')]
    ...
    def action_view_log_command_to_device(self):
        ...
        result = self.env['ir.actions.act_window']._for_xml_id(
            'pr_attendance_device.attendance_command_device_tree_action')
    ...
    def action_view_data_log_device(self):
        self.ensure_one()
        result = self.env['ir.actions.act_window']._for_xml_id(
            'pr_attendance_device.attendance_data_log_device_tree_action')
    ...
    def process_data_from_device(self, method, serial_number, data, table=None):
        ...
        """
        device = self.search(
            [('serialnumber', '=', serial_number), ('protocol', '=', 'icloud'), ('state', '=', 'confirmed')], limit=1)
        if device:
    ...
    def process_returned_command_results(self, serial_number, data):
        ...
        if cmd_device:
            ...
            if user_id:
                device_user = self.env['attendance.device.user'].with_context(active_test=False).search(
                    [('user_id', '=', user_id.group(1))], limit=1)
                if device_user:
            ...
            for user in users:
                ...
                if user_dict:
                    ...
                    data_list.append(User(uid=user_dict.get('Uid', None),
                                          ...
                                          privilege=int(user_dict.get('Pri', '')) if user_dict.get('Pri',
                                                                                                   '').isdigit() else None,
                                          password=str(user_dict.get('Passwd', '')),
                                          group_id=str(user_dict.get('Grp', '')),
                                          user_id=str(user_dict.get('USER PIN', '')),
                                          card=int(user_dict.get('Card', '')) if user_dict.get('Card',
                                                                                               '').isdigit() else -1, )
                                     )
        except Exception:
    ...
    def _download_operation_log(self, data_log):
        data_logs = self.env['attendance.data.log.device'].with_context(active_test=False).search(
            [('device_id', '=', self.id)])
