...
class access_management(models.Model):
    ...
    self_module_menu_ids = fields.Many2many('ir.ui.menu', 'access_management_ir_ui_self_module_menu',
                                            ...
                                            default=lambda self: self.env.ref(
                                                'pr_acess_managment.main_menu_pr_acess_managment'))
    ...
    hide_filters_groups_ids = fields.One2many('hide.filters.groups', 'access_management_id', 'Hide Filters/Group By',
                                              copy=True)
    is_apply_on_without_company = fields.Boolean(string="Apply Without Company", default=True,
    ...
    def get_remove_options(self, model):
        ...
        remove_action = self.env['remove.action'].sudo().search([('access_management_id.active', '=', True),
                                                                 ('access_management_id.user_ids', 'in',
                                                                  self.env.user.id),
                                                                 ('access_management_id', 'in',
                                                                  self.env.user.access_management_ids.ids),
                                                                 ('model_id.model', '=', model)])

        remove_action -= remove_action.filtered(lambda
    ...
    def get_chatter_hide_details(self, user_id, company_id, model=False):
        ...
        if model and hide_send_mail or hide_log_notes or hide_schedule_activity:
            ...
            hide_ids -= hide_ids.filtered(lambda
                                              x: x.access_management_id.is_apply_on_without_company == False and self.env.company.id not in x.access_management_id.company_ids.ids)
    ...
    def is_spread_sheet_available(self, action_model, action_id):
        ...
        restrict_spreadsheet -= restrict_spreadsheet.filtered(lambda
                                                                  x: x.access_management_id.is_apply_on_without_company == False and self.env.company.id not in x.access_management_id.company_ids.ids)
        if model:
    ...
    def is_export_hide(self, model=False):
        ...
        restrict_export -= restrict_export.filtered(lambda
                                                        x: x.access_management_id.is_apply_on_without_company == False and self.env.company.id not in x.access_management_id.company_ids.ids)
        if model:
            # if self.env['remove.action'].sudo().search([('access_management_id.active', '=', True),
            #                                      ('access_management_id.user_ids', 'in', self.env.user.id),
