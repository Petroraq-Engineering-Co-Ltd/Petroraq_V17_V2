...
from odoo.addons.pr_acess_managment.models.domain_prepare import prepare_domain_v2,compute_domain
import ast
...
class ir_rule(models.Model):
    ...
    def _compute_domain(self, model_name, mode="read"):
        ...
        read_value = True
        self._cr.execute("SELECT state FROM ir_module_module WHERE name='pr_acess_managment'")
        ...
        value = self._cr.execute(
            """SELECT value from ir_config_parameter where key='uninstall_pr_acess_managment' """)
        value = self._cr.fetchone()
        if not value:
            value = self._cr.execute("""select state from ir_module_module where name = 'pr_acess_managment'""")
            value = self._cr.fetchone()
