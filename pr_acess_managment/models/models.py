...
{resolution_info}""".format(operation_error=operation_error, resolution_info=resolution_info)
    ...
    def _get_view(self, view_id=None, view_type='form', **options):
        ...
        access_recs -= access_recs.filtered(lambda
                                                x: x.access_management_id.is_apply_on_without_company == False and self.env.company.id not in x.access_management_id.company_ids.ids)
        ...
        access_model_recs -= access_model_recs.filtered(lambda
                                                            x: x.access_management_id.is_apply_on_without_company == False and self.env.company.id not in x.access_management_id.company_ids.ids)
        ...
        if readonly_access_id:
            ...
            if access_model_recs:
                ...
                edit = 'true'
                create = 'true'
                default_move_type = self.env.context.get('default_move_type') or self.env.context.get(
                    'move_type') or 'entry'
                for access_model in access_model_recs:
                    if access_model.restrict_create:
