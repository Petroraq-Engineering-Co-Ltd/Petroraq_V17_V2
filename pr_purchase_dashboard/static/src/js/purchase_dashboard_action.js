...
export class PurchaseDashboardAction extends Component {
    setup() {
    ...
    async loadDashboardData() {
        ...
        this.state.data = await this.orm.call("pr.purchase.dashboard", "get_dashboard_data", []);
    ...
    linePoints(monthly = []) {
        ...
        return monthly.map((item, index) => {
            const x = pad + (index * (width - pad * 2)) / Math.max(monthly.length - 1, 1);
            const y = height - pad - ((Number(item.amount || 0) / max) * (height - pad * 2));
            return `${x},${y}`;
        }).join(" ");
    }
    ...
    sparklinePoints(monthly = []) {
        ...
        return monthly.map((item, index) => {
            ...
            const y = max ? height - ((Number(item.amount || 0) / max) * height) : height / 2;
            return `${x},${y}`;
        }).join(" ");
    }
}

PurchaseDashboardAction.template = "pr_purchase_dashboard.PurchaseDashboardAction";

registry.category("actions").add("pr_purchase_dashboard.purchase_dashboard", PurchaseDashboardAction);
